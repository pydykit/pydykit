# Found at
# https://github.com/kinnala/scikit-fem/blob/10.0.1/docs/examples/ex18.py
...
f = np.concatenate([asm(body_force, basis["u"]), basis["p"].zeros()])
...
if __name__ == "__main__":
    ...
    mesh.save(f"{name}_velocity.vtk", {"velocity": velocity[basis["u"].nodal_dofs].T})
    ...
    ax = draw(mesh)
    ax.tricontour(
        Triangulation(*mesh.p, mesh.t.T), psi[basis["psi"].nodal_dofs.flatten()]
    )
